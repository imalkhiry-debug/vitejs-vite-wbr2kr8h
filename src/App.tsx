...
export default function App() {
  const [page, setPage] = useState("home"); // home | register | admin | login
  ...
  const handleSubmit = async () => {
    ...
    const newEvent = { ...form, id: generateId(), status: "pending", createdAt: Date.now() };
  ...
  const toggleApprove = async (id) => {
    const updated = events.map(e => e.id === id ? { ...e, status: e.status === "approved" ? "pending" : "approved" } : e);
  ...
  const saveEdit = async () => {
    await saveEvents(events.map(e => e.id === editingId ? { ...editForm, id: editingId, status: e.status, createdAt: e.createdAt } : e));
    setEditingId(null);
  };
  ...
  return (
    <div style={styles.root}>
        ...
        <div style={styles.container}>
          ...
          <div style={styles.section}>
            ...
            {!loading && approvedEvents.length === 0 && (
              <p style={styles.empty}>لا توجد مناسبات معتمدة حتى الآن</p>
            )}
              ...
              {approvedEvents.map(ev => (
                <div key={ev.id} className="card" style={styles.card}>
                  ...
                  <div style={styles.cardRow}>
                    ...
                    <span>{ev.day}، {ev.hijriDate}</span>
                  </div>
                  <div style={styles.cardRow}>
                    <span style={styles.cardIcon}>📍</span>
                    <span>{ev.venue}</span>
                    {ev.mapLink && (
                      <a href={ev.mapLink} target="_blank" rel="noreferrer" style={styles.mapLink}>خريطة</a>
                    )}
                  </div>
                  <div style={styles.cardRow}>
                    <span style={styles.cardIcon}>📞</span>
                  ...
                  {ev.notes && (
                    <div style={styles.cardNotes}>{ev.notes}</div>
                  )}
                </div>
              ))}
            </div>
        ...
        <div style={styles.container}>
          <div style={styles.formWrap}>
            ...
            {formSuccess ? (
              ...
              <>
                <Field label="اسم صاحب المناسبة *" placeholder="مثال: فلان بن فلان الفلاني"
                  value={form.name} onChange={v => setForm({...form, name: v})} />
                <Field label="التاريخ الهجري *" placeholder="مثال: 1447/05/20"
                  value={form.hijriDate} onChange={v => setForm({...form, hijriDate: v})}
                  hint="يمكن الكتابة بأي صيغة: 1447/5/20 أو 20/5/1447" />
                ...
                <Field label="رقم الجوال للتواصل *" placeholder="0500000000"
                  value={form.phone} onChange={v => setForm({...form, phone: v})} />
                <Field label="عنوان المناسبة (القاعة / المدينة) *" placeholder="مثال: قاعة ليلتي، الرياض"
                  value={form.venue} onChange={v => setForm({...form, venue: v})} />
                <Field label="رابط موقع القاعة على الخريطة" placeholder="https://maps.google.com/..."
                  value={form.mapLink} onChange={v => setForm({...form, mapLink: v})} optional />
                <div style={styles.fieldGroup}>
                  <label style={styles.label}>ملاحظات إضافية <span style={styles.optional}>(اختياري)</span></label>
                  <textarea style={styles.textarea} placeholder="أي تفاصيل أخرى ترغب بإضافتها..."
      ...
      {page === "login" && (
        <div style={styles.container}>
          <div style={styles.formWrap}>
            ...
            <Field label="كلمة المرور" placeholder="أدخل كلمة المرور"
              value={adminPass} onChange={v => setAdminPass(v)} type="password" />
                ...
                {editingId === ev.id ? (
                  ...
                  <div style={styles.adminCardInner}>
                    <div style={styles.adminCardInfo}>
                      <span style={{
                        ...styles.statusBadge,
                        background: ev.status === "approved" ? "#d4edda" : "#fff3cd",
                        color: ev.status === "approved" ? "#155724" : "#856404"
                      }}>
                    ...
                    <div style={styles.adminActions}>
                      <button style={styles.editBtn} onClick={() => startEdit(ev)} title="تعديل">✏️</button>
                      <button style={{
                        ...styles.toggleBtn,
                        background: ev.status === "approved" ? "#fff3cd" : "#d4edda",
                        color: ev.status === "approved" ? "#856404" : "#155724"
                      }} onClick={() => toggleApprove(ev.id)} title={ev.status === "approved" ? "إخفاء" : "اعتماد"}>
                        {ev.status === "approved" ? "🚫" : "✅"}
                      </button>
                      <button style={styles.deleteBtn} onClick={() => deleteEvent(ev.id)} title="حذف">🗑️</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      <footer style={styles.footer}>
        منصة مناسبات — مساهمة مجتمعية
      </footer>
    </div>
  );
}

function Field({ label, placeholder, value, onChange, hint, optional, type = "text" }) {
  return (
    <div style={styles.fieldGroup}>
      <label style={styles.label}>
        {label} {optional && <span style={styles.optional}>(اختياري)</span>}
      </label>
...
const styles = {
  root: { fontFamily: "'Noto Kufi Arabic', 'Tajawal', sans-serif", direction: "rtl", minHeight: "100vh", background: "#f8f5f0", color: "#2c2416" },
  nav: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 24px", background: "#fff", borderBottom: "2px solid #e8d9b5", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 12px #0001" },
  navTitle: { fontSize: 20, fontWeight: 800, color: "#8b6914", letterSpacing: "0.01em" },
  ...
  heroTitle: { fontSize: 38, fontWeight: 900, color: "#7a5a10", margin: "0 0 8px", letterSpacing: "0.02em" },
  ...
  card: { background: "#fff", borderRadius: 16, padding: "20px 22px", border: "1px solid #e8d9b5", boxShadow: "0 2px 16px #0001", transition: "transform 0.2s, box-shadow 0.2s" },
  ...
  waBtn: { display: "flex", alignItems: "center", justifyContent: "center", width: 38, height: 38, borderRadius: "50%", background: "#25d366", color: "#fff", textDecoration: "none", flexShrink: 0, transition: "opacity 0.2s" },
  ...
  formWrap: { background: "#fff", borderRadius: 20, padding: "32px 28px", border: "1px solid #e8d9b5", boxShadow: "0 4px 24px #0001" },
  ...
  input: { width: "100%", padding: "11px 14px", border: "1.5px solid #e0cfa0", borderRadius: 10, fontSize: 14, background: "#fffdf7", color: "#2c2416", outline: "none", boxSizing: "border-box", fontFamily: "inherit", transition: "border 0.2s" },
  ...
  adminCard: { background: "#fff", borderRadius: 14, padding: "16px 20px", border: "1px solid #e8d9b5", marginBottom: 12, boxShadow: "0 1px 8px #0001" },
  ...
  statusBadge: { display: "inline-block", padding: "3px 10px", borderRadius: 20, fontSize: 12, fontWeight: 700, width: "fit-content" },
...
const css = `
  ...
  * { box-sizing: border-box; }
  .btn-gold {
    ...
    box-shadow: 0 4px 16px #c9a22740;
  }
  .btn-gold:hover { opacity: 0.9; transform: translateY(-1px); }
  .btn-gold:active { transform: translateY(0); }
  .card:hover { transform: translateY(-2px); box-shadow: 0 6px 24px #0002 !important; }
